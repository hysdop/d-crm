...
use Yii;
use yii\base\Object;
use frontend\modules\user\components\Helper;
...
class Assignment extends Object
{
    ...
    public function getItems()
    {
        ...
        foreach ($roles as $name => $item) {
            if (!in_array($item->name, ['root'])) {
                $avaliable[$name] = ['role', 'description' => $item->description];
            }
        }

        $permissions = $manager->getPermissions();
        foreach ($permissions as $name => $item) { //var_dump($item); die();
            if (!in_array($item->name, ['loginToBackend'])) {
                if ($name[0] != '/') {
        ...
        foreach ($manager->getAssignments($this->id) as $item) {
            if (!in_array($item->roleName, ['root', 'loginToBackend'])) {
