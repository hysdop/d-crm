...
- Enh: Custom side menu via `backend\modules\user\Module::items` (mdmunir).
...
- Chg: Can customize return of `backend\modules\user\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `backend\modules\user\components\BizRule` (mdmunir).
...
- Chg: Remove `backend\modules\user\Module::allowActions`. Set access control directly with `backend\modules\user\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `backend\modules\user\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
